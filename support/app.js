...
function renderMinerals() {
  ...
  root.innerHTML = minerals.map(m => {
    // tag pode ser string ou objeto {text, color}
    const tagText = (typeof m.tag === "string") ? m.tag : (m.tag?.text || "");
    const tagColor = (typeof m.tag === "string") ? (m.tagColor || "") : (m.tag?.color || m.tagColor || "");
    ...
    // Uso em feridas: decide pelo rótulo (mais confiável do que “ler” frases)
    const ttag = String(tagText || "").toLowerCase();
    const hasFW = !!tagText && !(
      ttag.includes("sem uso") ||
      ttag.includes("não curativo") || ttag.includes("nao curativo") ||
      ttag.includes("eletrônica") || ttag.includes("eletronica") ||
      ttag.includes("indireto")
    );
    ...
    if (hasDC && hasFW) dotBg = "linear-gradient(90deg, #D95B31 50%, #609CD2 50%)";
    else if (hasFW) dotBg = "#D95B31";
    else if (hasDC) dotBg = "#609CD2";

    const dc = dcList.map(x => `<li>${escapeHtml(x)}</li>`).join("");
 (m.emDataCenter || []).map(x => `<li>${escapeHtml(x)}</li>`).join("");
    ...
    return `
      <div class="mcard">
        <div class="mhead">
          ...
          ${
            tagText ? `<span class="tag"><span class="tag-dot" style="background:${dotBg}"></span>${escapeHtml(tagText)}</span>` : ""
          }
        </div>
