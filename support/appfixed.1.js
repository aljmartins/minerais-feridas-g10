...
function renderMinerals() {
  ...
  root.innerHTML = minerals.map(m => {
    const tag = m.tag || "";
    const hasDC = (m.emDataCenter || []).length > 0;
    const hasFW = (m.emFeridas || []).length > 0;

    // Cor da "bolinha" (tag-dot):
    // azul = só data center, laranja = só feridas, meio a meio = ambos, cinza = nenhum
    let dotBg = "rgba(11,18,32,0.35)";
    if (hasDC && hasFW) {
      dotBg = `linear-gradient(90deg, ${PALETTE.orange} 0 50%, ${PALETTE.blue} 50% 100%)`;
    } else if (hasFW) {
      dotBg = PALETTE.orange;
    } else if (hasDC) {
      dotBg = PALETTE.blue;
    }

    // Compatibilidade: se algum item tiver tagColor string explícita, use como fallback
    if (typeof m.tagColor === "string" && m.tagColor.trim()) {
      dotBg = m.tagColor.trim();
    }
    ...
    return `
      <div class="mcard">
        <div class="mhead">
          ...
          ${
            tag ? `<span class="tag"><span class="tag-dot" style="background:${dotBg}"></span>${escapeHtml(tag)}</span>` : ""
          }
        </div>
