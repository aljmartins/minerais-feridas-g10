...
function renderMinerals() {
  ...
  root.innerHTML = minerals.map(m => {
    ...
    const dcArr = (m.emDataCenter || []);
    const fwArr = (m.emFeridas || []);

    const hasDC = dcArr.length > 0;
    const hasFW = fwArr.length > 0;
    ...
    const dc = dcArr.map(x => `<li>${escapeHtml(x)}</li>`).join("");
    const fw = fwArr.map(x => `<li>${escapeHtml(x)}</li>`).join("");
